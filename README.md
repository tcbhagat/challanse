...
AWS deployment is currently frozen. The zero-budget, synthetic, local-LLM client demonstration environment is documented in [`docs/local-pilot.md`](docs/local-pilot.md). Beginner-safe execution steps and expected results are in [`docs/local-testing-runbook.md`](docs/local-testing-runbook.md), with daily operation and troubleshooting in [`docs/local-operations-manual.md`](docs/local-operations-manual.md).
